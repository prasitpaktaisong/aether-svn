...
int battle_delay_damage_sub(int tid, unsigned int tick, int id, intptr data)
{
	...
	{
		map_freeblock_lock();
		status_damage(dat->src,target,dat->damage,0,dat->delay,dat->reflected?64:0,dat->skill_id);
...
int battle_delay_damage(unsigned int tick, int amotion, struct block_list *src, struct block_list *target, int attack_type, int skill_id, int skill_lv, int damage, enum damage_lv dmg_lv, int ddelay, bool reflected)
{
	...
	{
		map_freeblock_lock();
		status_damage(src,target,damage,0,ddelay,reflected?64:0,skill_id);
	...
	if( sc && sc->count )
	{
		...
		if( sc->data[SC_SAFETYWALL] && (flag&(BF_SHORT|BF_MAGIC)) == BF_SHORT )
		{
			...
			if( group )
			{
				...
				status_change_end(bl,SC_SAFETYWALL,-1); // SafetyWall is gone! Why could this happend?
		}

		if( sc->data[SC_PNEUMA] && (skill_num == CR_ACIDDEMONSTRATION || (flag&(BF_MAGIC|BF_LONG)) == BF_LONG) )
		{
			d->dmg_lv = ATK_BLOCK;
			return 0;
		}
		...
		{ // SP Recovery of blocked magic damage is not working on official servers at the moment.
			d->dmg_lv = ATK_BLOCK;
			return 0;
		}
		...
		if(sc->data[SC_DODGE] && !sc->opt1 && (flag&BF_LONG || sc->data[SC_SPURT]) && rand()%100 < 20)
		{
			if( sd && pc_issit(sd) && !sc->data[SC_SITDOWN_FORCE] )
		...
		if( (sce = sc->data[SC_REJECTSWORD]) && flag&BF_WEAPON && skill_num != CR_ACIDDEMONSTRATION && (src->type != BL_PC || (((TBL_PC *)src)->status.weapon == W_DAGGER || ((TBL_PC *)src)->status.weapon == W_1HSWORD || ((TBL_PC *)src)->status.weapon == W_2HSWORD)) && rand()%100 < sce->val2 )
		{
			int rejected = 0;
			damage = damage * 50 / 100; // Damage is Reduced at Half when rejected
...
int battle_addmastery(struct map_session_data *sd,struct block_list *target,int dmg,int type)
{
	...
	if( (skill = pc_checkskill(sd,AL_DEMONBANE)) > 0 && target->type == BL_MOB && (battle_check_undead(status->race,status->def_ele) || status->race==RC_DEMON) )
		damage += (skill*(int)(3+(sd->status.base_level+1)*0.05));	// submitted by orn : damage += (skill * 3);
...
void battle_consume_ammo(TBL_PC*sd, int skill, int lv)
{
	...
	{ // Qty check should have been done in skill_check_condition
		...
		else if( sd->state.bg_id && map[sd->bl.m].flag.battleground )
...
static struct Damage battle_calc_weapon_attack(struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int wflag)
{
	...
	if( tstatus->mode&MD_PLANT ||
		(target->type == BL_SKILL && ((TBL_SKILL*)target)->group->skill_id == WM_REVERBERATION) ||
...
#define ATK_ADD2( a , b ) { wd.damage+= a; if (flag.lh) wd.damage2+= b; }
		...
		skillratio = skill_get_skillmod(skill_num, skill_lv, s_ele, wflag, src, sstatus, sc, target, tstatus, tsc);
...
static struct Damage battle_calc_weapon_attack_renewal(struct block_list *src, struct block_list *target, int skill_id, int skill_lv, int wflag)
{
	...
	flag.rh = 1;
	flag.weapon = 1;
		if( (tstatus->mode&MD_PLANT || (target->type == BL_SKILL && ((TBL_SKILL*)target)->group->unit_id == UNT_REVERBERATION))
		|| (target->type == BL_MOB && ((TBL_MOB*)target)->state.inmunity) )
		flag.infdef = 1;
	wd.type = 0; // Normal Attack
	...
	if( skill_id )
	{
		...
		case SR_GATEOFHELL:
			if( sd ) wd.div_ = sd->spiritball_old + 2; // 7 Consecutive hits.
			break;
		}
	}
	else
...
#define BON_ADD2( a , b ) { damage_bonus += a; if( flag.lh ) damage_bonus_ += b; }
	...
	if( flag.hit && !flag.infdef )
	{
		...
		case LG_EARTHDRIVE:
			...
			break;
		case RK_DRAGONBREATH:
			wd.damage = (status_get_hp(src) * 2 + status_get_max_sp(src) * 24) / 125;
			break;
		case SR_CRESCENTELBOW_AUTOSPELL:
			wd.damage = tstatus->max_hp * 3 / 10;
			break;
		...
		default:
			...
			{
			case RK_CRUSHSTRIKE:
				if( sd && (i = sd->equip_index[EQI_HAND_R]) >= 0 && sd->inventory_data[i] && sd->status.inventory[i].refine )
					wd.damage = wd.damage * sd->status.inventory[i].refine * (int)(3 * pow(2,sd->inventory_data[i]->wlv - 1));
				break;
			case LK_SPIRALPIERCE:
			case ML_SPIRALPIERCE:
				if( sd && (i = sd->equip_index[EQI_HAND_R]) >= 0 && sd->inventory_data[i] && sd->inventory_data[i]->type == IT_WEAPON )
					wd.damage += sd->inventory_data[i]->weight / 10; // 100% of Weight Damage - just a suggestion
				else
		...
		// SkillMod
		// -------------------------------------------------
		skillmod = skill_get_skillmod(skill_id, skill_lv, s_ele, wflag, src, sstatus, sc, target, tstatus, tsc);
	...
	if( flag.hit && !flag.infdef )
	{
		...
		case RA_WUGBITE:
			if( sd )
				i = 6 * pc_checkskill(sd, RA_TOOTHOFWUG);
			break;
		}
		BON_ADD(i);
...
#define MATK_ADD( a ) { ad.damage+= a; }
		...
		{	//Calc base damage according to skill
			...
			default:
			{
				...
				skillratio = skill_get_skillmod(skill_num, skill_lv, s_ele, mflag, src, sstatus, sc, target, tstatus, tsc);
...
struct Damage battle_calc_magic_attack_renewal(struct block_list *src, struct block_list *target, int skill_id, int skill_lv, int mflag)
{
	...
	if( !flag.infdef )
	{
		...
		default:
			...
			// -------------------------------------------------
			skillmod = skill_get_skillmod(skill_id, skill_lv, s_ele, mflag, src, sstatus, sc, target, tstatus, tsc);
		}
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int mflag)
{
	...
	if( md.damage < 0 )
		md.damage = 0;
	else if( md.damage && ((tstatus->mode&MD_PLANT && skill_num != RA_CLUSTERBOMB) || (target->type == BL_MOB && ((TBL_MOB*)target)->state.inmunity)) )
		md.damage = 1;
...
// BattleGround Settings
	...
	{ "bg_reserved_char_id",                &battle_config.bg_reserved_char_id,             0,      0,      INT_MAX,        },
...
// Ancient WoE
	{ "ancient_reserved_char_id",           &battle_config.ancient_reserved_char_id,        0,      0,      INT_MAX,        },
	{ "hunting_reserved_char_id",           &battle_config.hunting_reserved_char_id,        0,      0,      INT_MAX,        },
	{ "woe_reserved_char_id",               &battle_config.woe_reserved_char_id,            0,      0,      INT_MAX,        },
// PvP Event
...
// Custom TerraRO Settings
	...
	{ "mail_display_mode",                  &battle_config.mail_display_mode,               3,      1,      3,              },
