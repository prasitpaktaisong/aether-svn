...
static int pet_hungry(int tid, unsigned int tick, int id, intptr data)
{
	...
	switch(pd->pet.hungry) { //Informe de hambre de pet automatico. [Tab]
		case 75:
			snprintf(mes, sizeof mes,"%s : Sinto um pouco de fome...",pd->pet.name);
			clif_message(&pd->bl, mes);
		break;
		case 25:
			snprintf(mes, sizeof mes,"%s : Sinto fome...",pd->pet.name);
			clif_message(&pd->bl, mes);
		break;
		case 10:
			snprintf(mes, sizeof mes,"%s : Sinto muita fome...",pd->pet.name);
			clif_message(&pd->bl, mes);
		break;
	}
...
int pet_birth_process(struct map_session_data *sd, struct s_pet *pet)
{
	...
	clif_misceffect2(&sd->pd->bl, 344); // Efecto 2 de nacimiento [Tab]
	sprintf(pet_output,"%s!",pet->name); // Cuidado aca con el nombre del pet
