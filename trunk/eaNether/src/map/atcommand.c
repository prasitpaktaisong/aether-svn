...
ACMD_FUNC(showexp)
{
	if (sd->state.showexp) {
		sd->state.showexp = 0;
		clif_displaymessage(fd, "Indicadores de Experiência: Desligados");
		return 0;
	}

	sd->state.showexp = 1;
	clif_displaymessage(fd, "Indicadores de Experiência: Ligados");
	return 0;
}
...
ACMD_FUNC(showzeny)
{
	if (sd->state.showzeny) {
		sd->state.showzeny = 0;
		clif_displaymessage(fd, "Indicadores de Zeny: Desligados");
		return 0;
	}

	sd->state.showzeny = 1;
	clif_displaymessage(fd, "Indicadores de Zeny: Ligados");
	return 0;
}
...
ACMD_FUNC(exppenalty)
{
	...
	if (map[sd->bl.m].flag.noexppenalty) {
		clif_displaymessage(fd, "Penalidade de Experiência: Desativada");
		map[sd->bl.m].flag.noexppenalty = 0;
	} else {
		clif_displaymessage(fd, "Penalidade de Experiência: Ativada");
...
ACMD_FUNC(flooritem)
{
	...
	if( count != 0 )
		if( number == count )
			clif_displaymessage(fd, "Todos os Itens foram criados.");
		else
		{
			sprintf(atcmd_output, "%d Items criados.", count);
			clif_displaymessage(fd, atcmd_output);
		}
	else {
		clif_displaymessage(fd, "Nenhum Item criado!.");
		return -1;
	}
...
{
	pc_resetfeel(sd);
	clif_displaymessage(fd, "Percepções Solar, Lunar e Estelar reiniciadas.");

	return 0;
}

/*==========================================
 * Hate (SG mobs) Reset [Haziel]
 *------------------------------------------*/
ACMD_FUNC(hatereset)
{
	pc_resethate(sd);
	clif_displaymessage(fd, "Oposições Solar, Lunar e Estelar reiniciadas.");
...
ACMD_FUNC(aura)
{
	...
	type = atoi(message);
	sprintf(atcmd_output, "Aura Atual: %d", sd->state.user_aura);
	clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(mission)
{
	...
	{
		clif_displaymessage(fd, "Você não está Registrado no Sistema de Caça.");
		return 0;
	}
	...
	for( i = 0; i < 5; i++ )
	{
		...
		sprintf(atcmd_output, "- Caça: %d '%s' (ID : %d)", sd->hunting[i].count, mob_db(sd->hunting[i].mob_id)->jname, sd->hunting[i].mob_id);
		clif_displaymessage(fd, atcmd_output);
	...
	if( c < 1 )
		clif_displaymessage(fd, "Missão de Caça: Completa");
	else if( sd->hunting_time < (int)time(NULL) )
		clif_displaymessage(fd, "Missão de Caça: Falha");
	else
	{
 ...
 * @seeghp [Heaven & Rad]
 *------------------------------------------*/
int atcommand_seeghp(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
        nullpo_retr(-1, sd);
        if(!sd->status.guild_id){
                clif_displaymessage(sd->fd, "Você precisa de um Clã para utilizar este comando.");
                return -1;
        }
        if (!sd->state.seeghp) {
                sd->state.seeghp++;
                clif_displaymessage(sd->fd, "Você pode ver agora o HP dos membros do Clã e suas Alianças.");
        } else {
                sd->state.seeghp--;
...
ACMD_FUNC(order)
{
	...
	if( !message || !*message )
	{
		clif_displaymessage(fd, "Por favor, insira uma mensagem. (Uso: @order <mensagem>).");
		return -1;
	}
	...
	if( map[sd->bl.m].flag.battleground )
	{
		...
		{
			clif_displaymessage(fd, "Este Comando só pode ser utilizado por Líderes de Grupo nas Batalhas Campais.");
			return -1;
		}
	...
	else
	{
		...
		{
			clif_displaymessage(fd, "Este Comando só pode ser utilizado por Líderes de Guilda nas Guerras do Emperium.");
			return -1;
		}
...
ACMD_FUNC(reportafk)
{
	...
	//if( !sd->state.bmaster_flag )
		//clif_displaymessage(fd, "This command is reserved for Team Leaders Only.");
	if( !message || !*message )
		clif_displaymessage(fd, "Por favor, insira o nome do Personagem. (Uso: @reportafk <nome>).");
	else if( (pl_sd = map_nick2sd((char *)message)) == NULL )
		clif_displaymessage(fd, msg_txt(3)); // Character not found.
	else if( sd->state.bg_id != pl_sd->state.bg_id )
		clif_displaymessage(fd, "Este Personagem não pertence ao seu Time.");
	else if( sd == pl_sd )
		clif_displaymessage(fd, "Você não pode expulsar a sí mesmo.");
	else if( pl_sd->state.bg_afk == 0 )
		clif_displaymessage(fd, "Este Personagem não está Ausente.");
	else
	{ // Everytest OK!
		bg_team_leave(pl_sd,2);
		clif_displaymessage(pl_sd->fd, "Você foi expulso das Batalhas Campais por Ausência.");
		pc_setpos(pl_sd,pl_sd->status.save_point.map,pl_sd->status.save_point.x,pl_sd->status.save_point.y,3);

		sprintf(atcmd_output, "- O Jogador [%s], considerado Ausente, foi expulso das Batalhas Camapais.  -", pl_sd->status.name);
		clif_broadcast2(&sd->bl, atcmd_output, (int)strlen(atcmd_output)+1, 0xFF0000, 0x190, 20, 0, 0, BG);
		return 0;
	}
	return -1;
}
...
ACMD_FUNC(bgranked)
{
	...
	clif_displaymessage(fd, "============= RANKING DAS BATALHAS CAMPAIS ==============");
	for( i = 0; i < MAX_FAME_LIST && bgrank_fame_list[i].id; i++ )
	{
		sprintf(atcmd_output,"%d - %s - %d Pontos",i+1,bgrank_fame_list[i].name,bgrank_fame_list[i].fame);
		clif_displaymessage(fd, atcmd_output);
	}
	clif_displaymessage(fd, "============= RANKING DAS BATALHAS CAMPAIS ==============");
	return 0;
...
ACMD_FUNC(identifyall)
{
       ...
       clif_displaymessage(fd,(indentify) ? "Todos os Itens foram identificados." : "Todos os Itens estão identificados.");
       return 0;
}
...
int atcommand_ind_gstoragelock(const int fd, struct map_session_data* sd, const char* command, const char* message) {
	...
	if (sd->status.guild_id == 0 || (g = guild_search(sd->status.guild_id)) == NULL || strcmp(g->master,sd->status.name))
	{
		clif_displaymessage(fd, "Apenas o Líder da Guilda pode usar este comando.");
		return -1;
	}
	if (!message || !*message || sscanf(message, "%d %d", &ps,&perm) < 2) {
		int i;
		char outg[254];
		clif_displaymessage(fd, "Uso: @gstoragelock <Número da Posição, começando em 0> <1 para Permitido ou 0 para Proibido>");
		clif_displaymessage(fd, "Exemplo: @gstoragelock 1 1 irá permitir que a posição 1 (primeira além da Posição de Líder) utilize o @gstorage, enquanto @gstoragelock 1 0 o proibirá.");
		//display the permission list
		clif_displaymessage(fd,"======= Lista de Permissões =======");
		sprintf(outg,"[ %s (Líder) ]: Permitido (Fixo)",g->position[0].name);
		clif_displaymessage(fd,outg);
		for(i=1;i<MAX_GUILDPOSITION;i++){
			sprintf(outg,"[ %s->%d ] : %s",g->position[i].name,i,g->position[i].mode&0x20?"Permitido":"Proibido");
			clif_displaymessage(fd,outg);
		}
		clif_displaymessage(fd,"===================================");
		return -1;
	...
	if( ps < 1 || ps > MAX_GUILDPOSITION-1 ) {
		clif_displaymessage(fd,"A Posição inserida não existe ou é a Posição Líder.");
		return -1;
	}
	if( perm < 0 || perm > 1 ) {
		clif_displaymessage(fd,"A Permissão precisa ser 1, para Permitido ou 0, para Proibido.");
		return -1;
	}
	if( perm )
		allow = true;
	...
	clif_displaymessage(fd,"Permissão alterada com sucesso. Para ver a lista completa, utilize apenas '@gstoragelock'.");
	return 0;
}
...
AtCommandInfo atcommand_info[] = {
	...
	{ "feelreset",          1,1,    0,     atcommand_feelreset },
	{ "hatereset",          1,1,    0,     atcommand_hatereset },
	...
	{ "seeghp",             0,80,   0,     atcommand_seeghp },
	...
	{ "identifyall",        1,60,   0,	   atcommand_identifyall },
