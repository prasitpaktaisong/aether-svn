...
int mmo_char_fromsql(int char_id, struct mmo_charstatus* p, bool load_everything)
{
	...
	if( SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT "
		...
		"`save_map`,`save_x`,`save_y`,`partner_id`,`father`,`mother`,`child`,`fame`,`rename`,`playtime`"
	...
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 50, SQLDT_UINT,   &p->playtime, 0, NULL, NULL) )
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
		return 0;
...
int delete_char_sql(int char_id)
{
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `char_bg` WHERE `char_id` = '%d'", char_id) )
		Sql_ShowDebug(sql_handle);
	if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `char_bg_log` WHERE `char_id` = '%d'", char_id) )
		Sql_ShowDebug(sql_handle);
	if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `char_wstats` WHERE `char_id` = '%d'", char_id) )
		Sql_ShowDebug(sql_handle);
	if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `char_woe_log` WHERE `char_id` = '%d'", char_id) )
		Sql_ShowDebug(sql_handle);
