...
int pc_update_last_action(struct map_session_data *sd, int type)
{
	...
	{ // Battleground AFK announce
		char output[128];
		sprintf(output, "%s : %s n„o est· mais Ausente...", bg->g->name, sd->status.name);
...
int pc_reg_received(struct map_session_data *sd)
{
	...
	if( battle_config.bg_reward_rates != 100 )
	{
		...
		sprintf(output, "ï Batalhas Campais - Happy Hour - Recompensas em %d %% ï", erate);
...
int pc_dropitem(struct map_session_data *sd,int n,int amount)
{
	...
	if ((item_data= itemdb_exists(sd->status.inventory[n].nameid)) != NULL)
		if (item_data->type == IT_WEAPON || item_data->type == IT_CARD || item_data->type == IT_ARMOR) {
			clif_displaymessage (sd->fd, msg_txt(100));
		}
...
int pc_gainexp(struct map_session_data *sd, struct block_list *src, unsigned int base_exp,unsigned int job_exp, bool quest)
{
	...
	if(sd->state.showexp) {
		char output[256];
		sprintf(output,"ExperiÍncia Adquirida - Base: %u (%.2f%%) Classe: %u (%.2f%%)",base_exp,nextbp*(float)100,job_exp,nextjp*(float)100);
		clif_disp_onlyself(sd,output,strlen(output));
	}
...
void pc_calc_ranking(struct map_session_data *tsd, struct map_session_data *ssd, int skill)
{
	...
	if( map_allowed_woe(m) )
	{
		...
		{
			sprintf(output,"( VocÍ derrotou %s [%s] utilizando <%s> )", job_name(tsd->status.class_), tsd->status.name, ( skill ? skill_get_desc(skill) : "Melee/Reflect/Effect" ));
			clif_disp_onlyself(ssd,output,strlen(output));
		}
		if( tsd->state.battleinfo )
		{
			sprintf(output,"( %s [%s] derrotou vocÍ utilizando <%s> )", job_name(ssd->status.class_), ssd->status.name, ( skill ? skill_get_desc(skill) : "Melee/Reflect/Effect" ));
	...
	else if( map_pvpevent(m) )
	{
		...
		for( i = 0; i < sc; i++ )
		{
			...
			sprintf(output, "[%s] derrotado por [%s] - Obtidos %d / %d = %d Pontos. Stats PvP-Elo %d | Score %d.", tsd->status.name, ssd->status.name, Elo, sc, s_Elo, s_pl[i]->status.pvp.score, s_pl[i]->pvpevent_fame);
	...
	else if( ssd->state.bg_id && map[m].flag.battleground && tsd->state.bg_id )
	{
		...
		if( ssd->state.battleinfo )
		{
			sprintf(output,"( VocÍ derrotou %s [%s] utilizando <%s> )", job_name(tsd->status.class_), tsd->status.name, ( skill ? skill_get_desc(skill) : "Ataque FÌsico/Escudo Refletor/Status FÌsicos" ));
			clif_disp_onlyself(ssd,output,strlen(output));
		}
		if( tsd->state.battleinfo )
		{
			sprintf(output,"( %s [%s] derrotou vocÍ utilizando <%s> )", job_name(ssd->status.class_), ssd->status.name, ( skill ? skill_get_desc(skill) : "Ataque FÌsico/Escudo Refletor/Status FÌsicos" ));
			clif_disp_onlyself(tsd,output,strlen(output));
		}
...
//			ShowDebug("%s - Class %d: %u\n", type?"Job":"Base", job_id, max_level[job][type]);
	...
	for (; i <= MAX_LEVEL; i++)
		stats_point_table[i] = stats_point_table[i-1] + (i-1+15)/5;
