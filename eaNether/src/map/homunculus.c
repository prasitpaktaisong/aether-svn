...
int merc_hom_food(struct map_session_data *sd, struct homun_data *hd)
{
	...
	snprintf(mes, sizeof mes,"%s : Obrigado, mestre!!",hd->homunculus.name);
...
static int merc_hom_hungry(int tid, unsigned int tick, int id, intptr data)
{
	...
	if(hd->homunculus.hunger <= 10) {
		...
		snprintf(mes, sizeof mes,"%s : Sinto muita fome...",hd->homunculus.name);
	} else if(hd->homunculus.hunger == 25) {
		clif_emotion(&hd->bl, E_HMM);
		snprintf(mes, sizeof mes,"%s : Sinto fome...",hd->homunculus.name);
	} else if(hd->homunculus.hunger == 75) {
		clif_emotion(&hd->bl, E_OK);
		snprintf(mes, sizeof mes,"%s : Sinto um pouco de fome...",hd->homunculus.name);
	} else
		talk = false;
