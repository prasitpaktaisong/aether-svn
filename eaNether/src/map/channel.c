...
int channel_invite_timer(int tid, unsigned int tick, int id, intptr data)
{
	struct channel_data *cd;
	...
	{ // Channel commands
		...
		{ // Invite a User to the Channel
			...
			else
			{
				...
				p_sd->channel_invite_timer = add_timer(gettick() + 30000, channel_invite_timer, p_sd->bl.id, (intptr)aStrdup(cd->name));
			}
		}
